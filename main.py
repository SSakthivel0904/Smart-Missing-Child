...
class tk_master:
    ...
    def select_camera(self):
        ...
        def upload_files(event):
            tk_master.camera="Tenkasi"
            user_home_root.destroy()
            tt = tk_master
            tt.user_login(event)
        admin_id1 = canvas1.create_text(350, 170, text="Tenkasi", font=("Times New Roman", 28), fill=get_data.get_text_color())
        canvas1.tag_bind(admin_id1, "<1>", upload_files)

        def upload_files1(event):
            tk_master.camera="Pavoorchatram"
            user_home_root.destroy()
            tt = tk_master
            tt.user_login(event)
        admin_id2 = canvas1.create_text(350, 230, text="Pavoorchatram", font=("Times New Roman", 28), fill=get_data.get_text_color())
        canvas1.tag_bind(admin_id2, "<1>", upload_files1)

        def upload_files2(event):
            tk_master.camera="TVL"
            user_home_root.destroy()
            tt = tk_master
            tt.user_login(event)
        admin_id3 = canvas1.create_text(350, 260, text="TVL", font=("Times New Roman", 28), fill=get_data.get_text_color())
    ...
    def user_login(self):
        ...
        while True:
            ...
            if sts>15:
                ...
                msg['To'] = to_mail
                msg['Subject'] = "Child Face Detected on  Camera : " + str(tk_master.camera)
                file = str1
                fp = open(file, 'rb')
